...
## 2026-05-12 23:59 KST - Gemini CLI Code and System Full Audit

- Request: Use Gemini CLI to create a Korean full code and system audit report, then prioritize follow-up work.
- Change: Created a Gemini Pro audit prompt/report for local evidence, then moved the generated Gemini files to ignored local report storage so they are not part of the commit scope. Checked GitHub CLI authentication status and launched a visible `gh auth login` window for user-controlled browser authentication.
- Changed files: `docs/QUALITY_LOG.md`.
- Commands: `gemini --version`, `.\scripts\run-gemini-pro-analysis.ps1 -PromptFile ... -OutputPath ... -PrimaryModel gemini-3.1-pro-preview -FallbackModel gemini-2.5-pro`, `gh auth status -h github.com`, `gh repo view sheryloe/DonggriCompany --json ...`, `gh auth setup-git -h github.com`, `git ls-remote origin HEAD`, static `rg` inventory/path checks, `git check-ignore -v ...`.
- Validation: Gemini CLI `0.41.2` was available; primary model completed with `PrimaryCapacity429=False` and `FallbackUsed=False`; GitHub CLI authenticated as the repository owner account with admin repository permission; report secret-pattern scan found no common token value patterns; `D:\Donggr_Runtime` references were not found; `.env`, SQLite DB files, runtime folders, and generated artifacts are ignored by Git.
- Risk: GitHub authentication is complete, but no push/commit operation was performed. The Gemini report is a static audit and did not run install/build/test/Docker commands.
- Follow-up: Complete `gh auth login`, then run priority actions in order: secret/history scan, active legacy path classification, Gemini UTF-8 output hardening, and Project Health evidence regression monitoring.

## 2026-05-13 00:00 KST - Strategic Maintenance Department and Weekly Review Automation

- Request: Implement the official `strategic_maintenance` department, weekly review/report workflow, Gmail summary sending, and Settings UI controls.
- Change: Updated the organization manifest to `org-v5` with 8 canonical departments and 22 active staff, added idempotent department/agent backfill, protected the new department from deletion, added the `strategicMaintenance` settings contract, created the `strategic_maintenance_runs` table, implemented Markdown report generation, bounded improvement task creation, scheduler lifecycle wiring, Gmail send-scope support, API routes, OpenAPI docs, and the Settings `전략보수` tab.
- Changed files: `AGENTS.md`, `CHANGELOG.md`, `docs/OPERATIONS.md`, `docs/openapi.json`, `docs/strategic-maintenance.md`, `scripts/gmail-intake-oauth.mjs`, `server/messenger/gmail-client.ts`, `server/modules/bootstrap/schema/*`, `server/modules/strategic-maintenance/service.ts`, `server/modules/routes/ops/strategic-maintenance.ts`, `src/api/messaging-runtime-oauth.ts`, `src/components/settings/StrategicMaintenanceSettingsTab.tsx`, `src/types/index.ts`, related tests.
- Commands: `corepack pnpm exec vitest run server/modules/strategic-maintenance/service.test.ts server/messenger/gmail-client.test.ts server/modules/bootstrap/schema/seeds.test.ts server/modules/bootstrap/schema/organization-manifest.test.ts --config server/vitest.config.ts`, `corepack pnpm exec vitest run src/components/settings/StrategicMaintenanceSettingsTab.test.tsx src/components/settings/SettingsTabNav.test.tsx --config vitest.config.ts`, `corepack pnpm run openapi:sync`, `corepack pnpm run openapi:check`, `corepack pnpm run lint`, `corepack pnpm run build`, `git diff --check`.
- Validation: Targeted API/service/Gmail/org seed tests passed with 10 tests, targeted Settings UI tests passed with 3 tests, OpenAPI sync/check validated 119 operations, lint passed, production build passed, and whitespace diff check passed.
- Risk: Gmail report sending is intentionally blocked until recipients are configured and the OAuth grant includes `gmail.send`; automated code execution, commits, and pushes remain outside this workflow.
- Follow-up: Configure Gmail recipients in Settings and reconnect Gmail intake OAuth so the stored grant includes `gmail.send` before enabling email delivery.

## 2026-05-13 17:12 KST - Strategic Maintenance Runtime UI Activation

- Request: Continue the remaining work, start the server, and provide the running address.
- Change: Confirmed the local dev server was already running on ports 8800/8790, found the live UI still hid the new department behind 7-department frontend constants, then updated sidebar, office floor layout, manual text, organization defaults, department component IDs, subagent department types, room themes, and office-pack department copy for `strategic_maintenance`.
- Changed files: `src/components/Sidebar.tsx`, `src/components/office-view/officeFloorPlan.ts`, `src/components/ManualLibrary.tsx`, `src/components/OfficeView.tsx`, `src/components/DepartmentComponentsView.tsx`, `src/components/settings/OrganizationDefaultsTab.tsx`, `src/components/agent-manager/SubagentsTab.tsx`, `src/api/workflow-skills-subtasks.ts`, `server/modules/routes/ops/modules.ts`, `server/modules/routes/ops/subagents/catalog-snapshot.ts`, related tests.
- Commands: DB count query for departments/agents/settings, browser smoke against `http://127.0.0.1:8800`, targeted API/UI tests, `corepack pnpm run openapi:check`, `corepack pnpm run lint`, `corepack pnpm run build`, `git diff --check`.
- Validation: Runtime DB shows 8 departments, 3 strategic maintenance staff, `organizationSeedVersion=org-v5`, and default `strategicMaintenance` settings. Browser smoke shows sidebar `v5`, `8부서`, `직원 0/22 근무 중`, and the strategic maintenance staff `비컨`, `카이로`, `오빗`. Targeted tests, OpenAPI check, lint, production build, and whitespace check passed.
- Risk: Existing Gmail/Calendar intake tokens are expired or revoked, so the running server logs OAuth refresh failures until Gmail intake OAuth is reconnected.
- Follow-up: Re-run Gmail intake OAuth before enabling strategic maintenance email delivery.

## 2026-05-14 10:00 KST - Strategic Maintenance Gemini Verification and Full Test Gate

- Request: Ask Gemini CLI how to verify the strategic maintenance feature completely, then run the full feature test gate based on that analysis.
- Change: Created a Gemini Pro verification prompt/report for local evidence, moved the generated Gemini files to ignored local report storage, added an isolated strategic maintenance ops-route regression test, and fixed the `SkillsLibrary` ESC-close test mock so the newly rendered memory search panel APIs are covered during full Web tests.
- Changed files: `server/modules/routes/ops/strategic-maintenance.test.ts`, `src/components/SkillsLibrary.esc-close.test.tsx`, `docs/QUALITY_LOG.md`.
- Commands: `gemini --version`, `.\scripts\run-gemini-pro-analysis.ps1 -PromptFile .\docs\analysis\GEMINI_PRO_STRATEGIC_MAINTENANCE_VERIFICATION_PROMPT_2026-05-14.md -OutputPath .\docs\analysis\GEMINI_PRO_STRATEGIC_MAINTENANCE_VERIFICATION_2026-05-14.md -PrimaryModel gemini-3.1-pro-preview -FallbackModel gemini-2.5-pro -TimeoutSeconds 1200`, `corepack pnpm exec vitest run server/modules/routes/ops/strategic-maintenance.test.ts --config server/vitest.config.ts`, `corepack pnpm exec vitest run src/components/SkillsLibrary.esc-close.test.tsx --config vitest.config.ts`, `corepack pnpm run test:web`, `corepack pnpm run test:api`, `corepack pnpm run openapi:check`, `corepack pnpm run lint`, `corepack pnpm run build`, `git diff --check`, browser smoke against `http://127.0.0.1:8800`, `Invoke-RestMethod http://127.0.0.1:8790/api/health`.
- Validation: Gemini CLI `0.41.2` completed with `gemini-3.1-pro-preview`; the strategic maintenance route test passed with 2 tests; `SkillsLibrary.esc-close` passed with 9 tests; Web tests passed with 63 files and 184 tests; API tests passed with 99 files and 468 tests; OpenAPI check validated 119 operations; lint, production build, and whitespace diff check passed. Browser smoke confirmed the live UI shows `v5`, `8부서`, `직원 0/22 근무 중`, the `전략보수` department, and Settings > `전략보수` controls including weekly check, task creation, Gmail report, recipients/CC, recent/next run status, test email, manual check, and save. `/api/health` returned `ok=true`, version `2.0.4`.
- Risk: Live Gmail send was not executed because the current OAuth grant needs reconnect/send scope and recipients must be configured by the operator. A direct unauthenticated shell request to the protected strategic-maintenance status endpoint is expected to return unauthorized; the authenticated UI path and isolated route tests cover the feature behavior.
- Follow-up: Reconnect Gmail OAuth with `gmail.send`, configure recipients in Settings, then run one operator-approved live test email or manual report send.

## 2026-05-14 12:50 KST - Security Hardening Pass

- Request: Act on the identified security issues one by one and clean them up.
- Change: Restricted `/api/auth/session` bootstrap to actual loopback socket requests, changed CSRF tokens to per-process random values, kept browser CSRF/bearer auth state in memory only, removed GitHub token-in-URL clone/push usage with temporary `GIT_ASKPASS`, required configured allowed roots before deleting local clones, constrained task report document reads to safe files inside the task project, blocked custom API provider SSRF/private-network base URLs at settings and runtime use, and sanitized chat Markdown link protocols.
- Changed files: `server/security/auth.ts`, `src/api/core.ts`, `server/modules/shared/*`, `server/modules/routes/core/github-routes.ts`, `server/modules/workflow/core/worktree/merge.ts`, `server/modules/routes/ops/task-reports/helpers.ts`, `server/modules/routes/ops/api-providers.ts`, `server/modules/workflow/agents/providers/api-provider-tools.ts`, `server/modules/memory/store.ts`, `src/components/MessageContent.tsx`, related tests.
- Commands: `corepack pnpm test:api -- auth github-routes task-reports api-providers api-provider-tools`, `corepack pnpm test:web -- api MessageContent.security`, `corepack pnpm lint`, `corepack pnpm build`, `corepack pnpm run test:api`, `corepack pnpm run test:web`, `corepack pnpm run openapi:check`, additions-only secret-pattern scan, `git diff --check`.
- Validation: Targeted API/security tests passed with 9 files and 50 tests; targeted Web/API client tests passed with 6 files and 24 tests; lint passed; production build passed; full API tests passed with 99 files and 474 tests; full Web tests passed with 64 files and 187 tests; OpenAPI check validated 119 operations; additions-only secret-pattern scan found no matches; whitespace diff check passed.
- Risk: Existing API provider rows with blocked private/local URLs are now ignored or rejected at runtime except loopback Ollama; operators may need to update legacy custom providers to HTTPS public endpoints.
- Follow-up: Review the remaining lower-priority hardening candidates, especially public endpoint rate limiting/CSP and interrupt-token lifecycle tightening.

## 2026-05-20 01:08 KST - P0/P1 Security and Contract Closure

- Request: Continue by priority and move the system score closer to 100/100.
- Change: Enforced CSRF validation for cookie-authenticated mutating `/api/*` requests, added client retry on stale CSRF rejection, stopped `/api/settings` from returning decrypted messenger tokens, preserved encrypted messenger tokens when Settings saves redaction placeholders, aligned README/SECURITY/API docs with DonggriCompany `2.0.4` and org-v5 8-department/22-staff state, updated Swagger UI/OpenAPI examples to the DonggriCompany contract, added baseline CSP/security headers, rate-limited public session/inbox/OAuth endpoints, rejected private IPv6 API provider URLs, and blocked provider-fetch redirects.
- Changed files: `server/security/auth.ts`, `src/api/core.ts`, `server/messenger/token-crypto.ts`, `server/modules/routes/ops/settings-stats.ts`, `server/modules/routes/ops/api-docs.ts`, `server/modules/routes/ops/api-providers.ts`, `server/modules/shared/api-provider-url-policy.ts`, `server/modules/workflow/agents/providers/api-provider-tools.ts`, `server/modules/memory/store.ts`, `scripts/openapi-contract.mjs`, `README.md`, `SECURITY.md`, `docs/api.md`, `docs/openapi.json`, related tests.
- Commands: `corepack pnpm exec vitest run server/security/auth.test.ts --config server/vitest.config.ts`, `corepack pnpm exec vitest run src/api.test.ts --config vitest.config.ts`, `corepack pnpm exec vitest run server/modules/routes/ops/settings-stats.test.ts --config server/vitest.config.ts`, `corepack pnpm exec vitest run server/modules/routes/ops/api-providers.test.ts --config server/vitest.config.ts`, `corepack pnpm exec vitest run server/modules/workflow/agents/providers/api-provider-tools.test.ts server/modules/routes/core/memory.test.ts --config server/vitest.config.ts`, `corepack pnpm run openapi:sync`, `corepack pnpm run openapi:check`, `corepack pnpm run test:api`, `corepack pnpm run test:web`, `corepack pnpm run lint`, `corepack pnpm run build`, additions-only secret-value scan, `git diff --check`.
- Validation: Targeted CSRF/API/settings/provider tests passed with 62 tests; OpenAPI check validated 119 operations; full API tests passed with 99 files and 480 tests; full Web tests passed with 64 files and 188 tests; lint passed; production build passed; secret-value scan found no matches; whitespace diff check passed.
- Risk: Gmail live sending still requires operator OAuth reconnect with `gmail.send` and configured recipients. Existing D-drive strings found in `ProjectInsightsPanel` tests are fixtures, not active runtime paths.
- Follow-up: Address remaining P1/P2 items: live Gmail smoke after OAuth reconnect and resolver-level DNS revalidation for custom provider hostnames if future threat modeling requires protection against DNS rebinding.

## 2026-05-21 19:45 KST - Donggri 8-Bit Agents UI/UX v1

- Request: Implement a Donggri-owned 8-bit agent UI/UX upgrade inspired by the AWS 8-Bit Agents concept without copying AWS assets.
- Change: Added `pixelAgentMode` settings defaults, a Settings `픽셀 에이전트` tab, Office pixel mode propagation, an abstract Pixi cloud-lab layer, pixel badges on agent cards/details, and a design-guide section documenting the asset and brand policy.
- Changed files: `src/types/index.ts`, `src/app/utils.ts`, `src/components/settings/*`, `src/components/OfficeView.tsx`, `src/components/office-view/*`, `src/components/agent-manager/*`, `src/components/AgentDetail.tsx`, `src/styles/index.pixel-agents.css`, `docs/DESIGN.md`, `docs/QUALITY_LOG.md`.
- Commands: `corepack pnpm exec vitest run src/components/settings/PixelAgentModeSettingsTab.test.tsx src/components/settings/SettingsTabNav.test.tsx --config vitest.config.ts`, `corepack pnpm run agents:sprites:check`, `corepack pnpm run lint`, `corepack pnpm run build`, `corepack pnpm run test:web`, `corepack pnpm run test:api`, `corepack pnpm run openapi:check`, `git diff --check`, Playwright smoke against `http://127.0.0.1:8800`.
- Validation: Settings targeted tests passed with 2 tests; sprite check passed with 528 files inspected and 0 errors; lint passed; production build passed; Web tests passed with 65 files and 189 tests; API tests initially hit one Windows temp-folder timeout in `cli-runtime.account-pool.test.ts`, then the targeted rerun passed with 10 tests and full API rerun passed with 99 files and 480 tests; OpenAPI check validated 119 operations; whitespace diff check passed. Playwright confirmed desktop/mobile Office pixel mode with one Pixi canvas, visible pixel-mode banner, `image-rendering: pixelated`, and nonblank canvas screenshots.
- Risk: Existing security-patch dirty files are intentionally preserved; AWS references are inspiration only and no AWS logos/icons/characters are embedded. Browser smoke used a mocked Settings GET to force `pixelAgentMode.enabled=true` without changing server settings.
- Follow-up: If this direction is accepted, add deterministic sprite/event variations through the existing generator pipeline rather than hand-editing runtime PNG files.
