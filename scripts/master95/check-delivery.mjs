...
const candidateFormatDebtPath = resolve(repoRoot, "contracts/v1/FORMAT_DEBT_BASELINE.json");
const formatDebtPath = process.env.DONGRI_FORMAT_DEBT_BASELINE_PATH
  ? resolve(process.env.DONGRI_FORMAT_DEBT_BASELINE_PATH)
  : existsSync(candidateFormatDebtPath)
    ? candidateFormatDebtPath
    : null;
...
if (existsSync(formatFingerprintPath)) {
  ...
  if (formatDebtPath && existsSync(formatDebtPath)) {
    ...
    warnings.push(
      "candidate-specific format debt baseline was not supplied; the checked-in standalone fingerprint was enforced",
    );
  }
}
