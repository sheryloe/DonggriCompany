...
describe("reply core safe fallback", () => {
  it("returns department-specific Korean planned feedback without mojibake", () => {
    ...
    const reply = tools.chooseSafeReply({ text: "" }, "ko", "feedback", {
      id: "dev-lead",
      name: "Aria",
      name_ko: "아리아 (개발팀장)",
  ...
  it("uses English fallback for non-Korean locales", () => {
    const tools = makeTools();
    const reply = tools.chooseSafeReply({ text: "" }, "ja", "feedback", {
      id: "qa-lead",
      name: "Hawk",
      name_ko: "호크 (QA팀장)",
      department_id: "qa",
  ...
  it("returns readable Korean failure messages", () => {
    const tools = makeTools();
    const reply = tools.chooseSafeReply({ text: "", error: "timeout after 100ms" }, "ko", "feedback", {
      id: "qa-lead",
      name: "Hawk",
      name_ko: "호크 (QA팀장)",
      department_id: "qa",
