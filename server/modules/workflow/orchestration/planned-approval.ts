  ...
  function parseApprovalGateState(raw?: string | null): { gates: string[]; blockedBy: string[] } {
    ...
    try {
      ...
      const gates = Array.isArray(parsed?.gates)
        ? parsed.gates.map((g: unknown) => String(g ?? "").trim()).filter(Boolean)
        : [];
  ...
  function startPlannedApprovalMeeting(
    ...
    void (async () => {
      ...
      try {
        ...
        const taskCtx = db.prepare(`SELECT ${taskSelectColumns.join(", ")} FROM tasks WHERE id = ?`).get(taskId) as
          | {
        ...
        const wantsRevision = (content: string): boolean =>
          /보완|수정|보류|리스크|추가.*필요|hold|revise|revision|required|pending|risk|block|保留|修正|追加|风险|風險/i.test(
            content,
          );
        ...
        const buildFallbackPublicFeedback = (leader: any): string => {
          ...
          const koByDept: Record<string, string> = {
            ...
            operations:
              "운영은 실행 경로와 실패 시 재시도 절차를 확인하겠습니다. 산출물은 실행 절차와 장애 대응 메모입니다.",
            "cicd-repo":
          ...
          const ko =
            koByDept[normalizedDeptId] ??
            `${deptName}는 담당 범위를 구체 작업으로 나누고 산출물과 완료 기준을 함께 보고하겠습니다.`;
          const en =
