...
const SUBTASK_TOKEN_REGEX = /(sub[\s_-]*task|\uC11C\uBE0C\uD0DC\uC2A4\uD06C|\uC3D5\uD23F|\?\uC3D5\uD23F|\?쒕툕|쒕툕)/i;
...
export function normalizeSubtaskTitleForStorage(input: unknown): string {
...
export function normalizeSubtaskTitleForDisplay(input: unknown): string {
  return normalizeSubtaskTitleForStorage(input);
}
