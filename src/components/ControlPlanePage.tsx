...
export function isProjectionBoundary(expectedProjectionEpoch: string | null, nextProjectionEpoch: string): boolean {
  return expectedProjectionEpoch !== null && expectedProjectionEpoch !== nextProjectionEpoch;
}
...
export default function ControlPlanePage({ initialTab = "root", compactHeader = false }: ControlPlanePageProps) {
  ...
  const [selectedActiveSpecId, setSelectedActiveSpecId] = useState<string | null>(null);
  ...
  const load = async (expectedProjectionEpoch: string | null = projectionEpochRef.current) => {
    ...
    setError(null);
    try {
      const [nextState, nextThreadInfo] = await Promise.all([
        getControlPlaneState(controller.signal),
        getCodexThreadCurrent(controller.signal),
      ]);
      if (controller.signal.aborted || sequence !== loadSequenceRef.current) return;
      // The sequence guard rejects older in-flight reads. A different mutable
      // projection epoch on the newest response is a normal root-document
      // boundary. Clear results derived from the previous projection before
      // promoting the newest state so the UI never mixes two projections.
      if (isProjectionBoundary(expectedProjectionEpoch, nextState.projection_epoch)) {
        setSyncResult(null);
        setRunnerResult(null);
        setThreadResult(null);
        setSelectedScopeId(null);
        selectedScopeRef.current = null;
        setSelectedActiveSpecId(null);
        setMemoryResult(null);
        setMemoryContextResult(null);
        setMemoryRememberResult(null);
        setHarnessResult(null);
        setEnginePreview(null);
        setEngineRunResult(null);
      }
      projectionEpochRef.current = nextState.projection_epoch;
      setState(nextState);
  ...
  useEffect(() => {
    if (!selectedScopeId || selectedScopeRef.current === selectedScopeId) return;
    selectedScopeRef.current = selectedScopeId;
    void load(projectionEpochRef.current);
  }, [selectedScopeId]);
  ...
  const authoritativeActiveSpecs =
    state.active_specs.length > 0 ? state.active_specs : state.active_spec ? [state.active_spec] : [];
  const projectActiveSpec = authoritativeActiveSpecs.find((spec) =>
    [spec.related_repo, ...spec.related_repos]
      .filter((repo): repo is string => Boolean(repo))
      .some((repo) => {
        const normalized = repo.replace(/\\/g, "/").replace(/\/+$/, "");
        return normalized === selectedProjectKey || normalized.endsWith(`/${selectedProjectKey}`);
      }),
  );
  const selectedActiveSpec =
    authoritativeActiveSpecs.find((spec) => spec.id === selectedActiveSpecId) ??
    projectActiveSpec ??
    authoritativeActiveSpecs[0] ??
    state.active_spec;
  ...
  const threadScopeValue =
    threadScope === "root" ? null : threadScope === "spec" ? (selectedActiveSpec.id ?? null) : selectedProjectKey;
  const finishSpecId = getActivationSpecId(sameThreadActivation) ?? selectedActiveSpec.id;
        ...
        <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
          <Panel title="Root 상태" eyebrow="source of truth">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Metric
                label="Active Spec"
                value={selectedActiveSpec.id ?? "-"}
                hint={selectedActiveSpec.phase ?? undefined}
              />
            </div>
            <div className="mt-3 space-y-2" data-testid="active-spec-authority">
              <label className="block text-xs font-semibold" htmlFor="active-spec-selector">
                활성 Spec 선택 ({authoritativeActiveSpecs.length})
              </label>
              <select
                id="active-spec-selector"
                aria-label="활성 Spec 선택"
                value={selectedActiveSpec.id ?? ""}
                onChange={(event) => setSelectedActiveSpecId(event.target.value || null)}
                className="w-full rounded-lg border px-3 py-2 font-mono text-xs outline-none focus:border-cyan-400/60"
                style={{
                  borderColor: "var(--th-border)",
                  background: "var(--th-bg-surface)",
                  color: "var(--th-text-primary)",
              ...
              >
                {authoritativeActiveSpecs.map((spec, index) => (
                  <option key={spec.id ?? `active-spec-${index}`} value={spec.id ?? ""}>
                    {spec.id ?? "(invalid spec)"} · {spec.status ?? "unknown"} · {spec.phase ?? "unknown"}
                  </option>
                ))}
              </select>
              <div className="flex flex-wrap gap-2" aria-label="전체 활성 Spec 목록">
                {authoritativeActiveSpecs.map((spec, index) => (
                  <Pill key={spec.id ?? `active-spec-pill-${index}`} ok={!spec.parse_error}>
                    {spec.id ?? "(invalid spec)"}
                  </Pill>
                ))}
              </div>
            </div>
                  ...
                  void runAction(async () => {
                    ...
                    setMemoryRememberResult(
                      await rememberAgentMemory({
                        ...
                        scope: memoryScope,
                        spec_id: selectedActiveSpec.id ?? undefined,
