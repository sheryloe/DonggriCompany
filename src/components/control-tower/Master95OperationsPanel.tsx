...
import {
  executeDurableControlTowerMutation,
  ...
  type PreparedControlTowerMutation,
...
export default function Master95OperationsPanel({
  ...
  const [pendingMutation, setPendingMutation] = useState<PreparedControlTowerMutation | null>(null);
  ...
  const [streamReason, setStreamReason] = useState<"connected" | "journey" | "action" | "projection" | null>(null);
  ...
  const candidateSourceEpochRef = useRef<string | null>(null);
  const projectionEpochRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    activeProjectRef.current = subscribedProjectId;
    candidateSourceEpochRef.current = null;
    projectionEpochRef.current = null;
    ...
    void subscribeDurableControlTowerState(activeProjectId, {
      ...
      onSnapshot(event) {
        if (disposed || activeProjectRef.current !== subscribedProjectId) return;
        if (candidateSourceEpochRef.current && event.source_epoch !== candidateSourceEpochRef.current) {
          setDurableError("Control Plane source epoch이 변경되어 이전 실시간 응답을 폐기했습니다.");
          return;
        }
        if (projectionEpochRef.current && event.projection_epoch !== projectionEpochRef.current) {
          setDurableError("Control Plane projection epoch이 다른 이전 실시간 응답을 폐기했습니다.");
          return;
        }
        candidateSourceEpochRef.current = event.source_epoch;
        projectionEpochRef.current = event.projection_epoch;
      ...
      onReset(event) {
        if (disposed || activeProjectRef.current !== subscribedProjectId) return;
        if (candidateSourceEpochRef.current && event.source_epoch !== candidateSourceEpochRef.current) {
          setDurableError("Release candidate source epoch drift를 차단했습니다.");
          return;
        }
        candidateSourceEpochRef.current = event.source_epoch;
        projectionEpochRef.current = event.projection_epoch;
        setDurableState(null);
        setDurableNotice(`실시간 cursor가 ${event.reason} 상태여서 현재 Project 상태를 다시 불러와야 합니다.`);
      },
    })
      .then((close) => {
        if (disposed) close?.();
  ...
  const pendingScope =
    pendingMutation?.preview.scope &&
    typeof pendingMutation.preview.scope === "object" &&
    !Array.isArray(pendingMutation.preview.scope)
      ? (pendingMutation.preview.scope as Record<string, unknown>)
      : null;

  function switchProject(projectIdToSelect: string) {
    activeProjectRef.current = projectIdToSelect;
    candidateSourceEpochRef.current = null;
    projectionEpochRef.current = null;
    stateRequestSequenceRef.current += 1;
    stateAbortRef.current?.abort();
  ...
  async function refreshDurableState() {
    const requestedProjectId = durableProjectId;
    ...
    setDurableError(null);
    try {
      const state = await readDurableControlTowerState(requestedProjectId, controller.signal);
      if (
        controller.signal.aborted ||
        requestSequence !== stateRequestSequenceRef.current ||
        activeProjectRef.current !== requestedProjectId
      ) {
        return;
      }
      if (state.root_project_id !== requestedProjectId) {
        setDurableError("Project 범위가 다른 재시작 응답을 차단했습니다.");
        return;
      }
      if (candidateSourceEpochRef.current && state.source_epoch !== candidateSourceEpochRef.current) {
        setDurableError("Control Plane source epoch이 변경되어 이전 재시작 응답을 폐기했습니다.");
        return;
      }
      const projectionChanged =
        Boolean(projectionEpochRef.current) && state.projection_epoch !== projectionEpochRef.current;
      candidateSourceEpochRef.current = state.source_epoch;
      projectionEpochRef.current = state.projection_epoch;
      setDurableState(state);
      setDurableNotice(
        projectionChanged
          ? `Control Plane projection 경계를 감지해 최신 상태로 자동 재동기화했습니다. Artifact ${state.artifacts.length}개 · event ${state.event_count}개`
          : `재시작 복구 상태를 불러왔습니다. Artifact ${state.artifacts.length}개 · event ${state.event_count}개`,
      );
    } catch (error) {
      if (controller.signal.aborted || requestSequence !== stateRequestSequenceRef.current) return;
      setDurableError(error instanceof Error ? error.message : String(error));
    } finally {
      if (requestSequence === stateRequestSequenceRef.current) setDurableLoading(null);
    }
  }
  ...
  async function executeDurableJourney(journeyId: ControlTowerJourneyId) {
    ...
    try {
      const label = journeys.find((journey) => journey.id === journeyId)?.title ?? journeyId;
      const prepared = await runDurableControlTowerJourney(durableProjectId, journeyId, label);
      setPendingMutation(prepared);
      setConfirmationText("");
      setDurableNotice(`${label} preview와 승인 영수증을 발급했습니다. 확인문을 직접 입력해야만 실행됩니다.`);
    } catch (error) {
      setDurableError(error instanceof Error ? error.message : String(error));
    } finally {
      setDurableLoading(null);
  ...
  async function executeDurableAction(actionId: ControlTowerActionId, targetId: string, label: string, value?: string) {
    ...
    setDurableError(null);
    try {
      const prepared = await runDurableControlTowerAction(durableProjectId, actionId, targetId, value, label);
      setPendingMutation(prepared);
      setConfirmationText("");
      setDurableNotice(`${label} preview와 승인 영수증을 발급했습니다. 확인문을 직접 입력해야만 실행됩니다.`);
    } catch (error) {
  ...
  async function confirmPendingMutation() {
    if (!pendingMutation || confirmationText !== pendingMutation.preview.confirmation_text) return;
    if (pendingMutation.preview.project_id !== activeProjectRef.current) {
      setDurableError("Project 전환 뒤의 이전 mutation preview는 실행할 수 없습니다.");
      return;
    }
    setDurableLoading("state");
    setDurableError(null);
    try {
      const response = await executeDurableControlTowerMutation(pendingMutation, confirmationText);
      if (activeProjectRef.current !== pendingMutation.preview.project_id) return;
      setDurableState(response.result.snapshot);
      candidateSourceEpochRef.current = pendingMutation.preview.source_epoch;
      setDurableNotice(
        `${pendingMutation.label} 완료 · 외부 효과 없음 · event ${response.result.snapshot.event_count}개`,
      );
      setPendingMutation(null);
      setConfirmationText("");
    } catch (error) {
  ...
  return (
    <section
      ...
      data-testid="master95-operations-panel"
      style={{ "--th-text-muted": "var(--th-text-secondary)" } as CSSProperties}
    >
        ...
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(14rem,0.7fr)] md:items-end">
          ...
          <label className="text-xs font-semibold" htmlFor="control-tower-project-switcher">
            ...
            <select
              ...
              disabled={durableLoading !== null && durableLoading !== "state"}
      ...
      {pendingMutation ? (
        <section
          className="mt-4 rounded-lg border border-amber-500/50 bg-amber-400/10 p-4"
          aria-labelledby="control-tower-mutation-approval-heading"
          data-testid="control-tower-v2-approval"
        >
          <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-amber-800 dark:text-amber-100">
            v2 mutation · manual confirmation
          </div>
          <h3 id="control-tower-mutation-approval-heading" className="mt-1 text-sm font-bold">
            {pendingMutation.label} 실행 승인
          </h3>
          <dl className="mt-3 grid gap-2 text-xs md:grid-cols-2">
            <div>
              <dt className="font-semibold">작업</dt>
              <dd className="break-all font-mono">{pendingMutation.preview.operation}</dd>
            </div>
            <div>
              <dt className="font-semibold">대상</dt>
              <dd className="break-all font-mono">{pendingMutation.preview.resolved_target}</dd>
            </div>
            <div>
              <dt className="font-semibold">범위</dt>
              <dd className="break-all font-mono">{JSON.stringify(pendingMutation.preview.scope)}</dd>
            </div>
            <div>
              <dt className="font-semibold">만료</dt>
              <dd>{formatTimestamp(pendingMutation.preview.expires_at)}</dd>
            </div>
            <div>
              <dt className="font-semibold">외부 효과</dt>
              <dd>{pendingScope?.external_effect === false ? "없음" : "확인 필요"}</dd>
            </div>
            <div>
              <dt className="font-semibold">백업 / rollback</dt>
              <dd>
                {String(pendingScope?.backup ?? "미기록")} / {String(pendingScope?.rollback ?? "미기록")}
              </dd>
            </div>
            <div className="md:col-span-2">
              <dt className="font-semibold">승인 영수증 SHA-256</dt>
              <dd className="break-all font-mono">{pendingMutation.approval_receipt.receipt_sha256}</dd>
            </div>
          </dl>
          <p id="control-tower-required-confirmation" className="mt-3 text-xs">
            아래 확인문을 직접 입력하세요. 자동 입력·자동 실행은 지원하지 않습니다.
          </p>
          <code className="mt-2 block break-all rounded border p-2 text-xs">
            {pendingMutation.preview.confirmation_text}
          </code>
          <label className="mt-3 block text-xs font-semibold" htmlFor="control-tower-confirmation-input">
            확인문 직접 입력
          </label>
          <input
            id="control-tower-confirmation-input"
            value={confirmationText}
            onChange={(event) => setConfirmationText(event.target.value)}
            autoComplete="off"
            spellCheck={false}
            aria-describedby="control-tower-required-confirmation"
            className="mt-1 block w-full rounded-lg border bg-transparent px-3 py-2 font-mono text-sm"
          />
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              type="button"
              className={controlButtonClass}
              disabled={durableLoading !== null || confirmationText !== pendingMutation.preview.confirmation_text}
            ...
            <button
              ...
              onClick={() => {
                setPendingMutation(null);
                setConfirmationText("");
                setDurableNotice("mutation preview를 취소했습니다. 변경은 실행되지 않았습니다.");
              }}
            >
              취소
            </button>
