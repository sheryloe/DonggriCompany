...
function buildFallbackCommandTimeline(tasks: Task[]) {
  ...
  for (const task of tasks) {
    ...
    if (typeof taskAny.department_id === "string" && taskAny.department_id && !summary.departments.includes(taskAny.department_id)) {
    ...
    if (Number(taskAny.subtask_total ?? 0) > 0 || taskAny.source_task_id || String(taskAny.title ?? "").includes("서브태스크")) {
...
export function DashboardCommandTimeline({ timeline, tasks, language }: { timeline?: any[]; tasks: Task[]; language: UiLanguage }) {
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/70 p-5 shadow-2xl shadow-slate-950/40 backdrop-blur-xl">
      <div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          <div className="text-[11px] font-black uppercase tracking-[0.32em] text-amber-200/80">CEO Project Command Timeline</div>
          <h2 className="mt-2 text-2xl font-black text-white">CEO 명령 프로젝트 진행판</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
            계획, 회의, 분업, 진행, 검토, 완료, 보고까지 프로젝트별 흐름을 한 화면에 모읍니다. 회의 공개 발언,
            텔레그램 보고 성공, 리뷰 하드블록 메타를 함께 집계합니다.
          </p>
        </div>
        <div className="grid min-w-0 grid-cols-3 gap-2 text-center">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-3">
            <div className={blockedProjects > 0 ? "text-xl font-black text-rose-200" : "text-xl font-black text-emerald-200"}>{reportCount}</div>
      ...
      <div className="mt-5 grid min-w-0 gap-3">
        {rows.length === 0 ? (
          ...
          rows.map((row) => (
            <article key={row.project_id ?? "unassigned"} className="min-w-0 rounded-3xl border border-white/10 bg-white/[0.04] p-4">
              <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <h3 className="min-w-0 truncate text-lg font-black text-white">{row.project_name}</h3>
                    <span className={`rounded-full border px-2.5 py-1 text-[10px] font-black ${commandHealthClass(row.health)}`}>
                  ...
                  <p className="mt-1 min-w-0 truncate text-xs text-slate-400">{row.latest_task_title ?? "최근 태스크 없음"}</p>
                </div>
                <div className="shrink-0 text-xs font-bold text-slate-400">최근 갱신 {formatCommandTime(row.latest_at)}</div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className={active ? "text-sm font-black text-white" : "text-sm font-black text-slate-500"}>{COMMAND_LANE_LABELS[key]}</span>
                        <span className={active ? "h-2 w-2 rounded-full bg-emerald-300" : "h-2 w-2 rounded-full bg-slate-700"} />
                      </div>
                      <div className="mt-2 text-xl font-black text-white">{lane.count ?? 0}</div>
                      <div className="mt-1 text-[10px] font-bold text-slate-400">{formatCommandTime(lane.latest_at)}</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-4 flex min-w-0 flex-wrap gap-2 text-[11px] font-bold text-slate-300">
                <span className="rounded-full border border-white/10 bg-slate-900 px-3 py-1">공개 발언 {row.signal_counts?.meeting_public_feedback ?? 0}</span>
                <span className="rounded-full border border-white/10 bg-slate-900 px-3 py-1">텔레그램 보고 {row.signal_counts?.messenger_relay_success ?? 0}</span>
                <span className="rounded-full border border-white/10 bg-slate-900 px-3 py-1">리뷰 차단 {row.signal_counts?.review_consent_blocked ?? 0}</span>
                {(row.departments ?? []).slice(0, 6).map((departmentId: string) => (
                  <span key={departmentId} className="rounded-full border border-sky-400/20 bg-sky-500/10 px-3 py-1 text-sky-100">
