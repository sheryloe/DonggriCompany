    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.2fr_1fr]">
      <div className="game-panel p-5">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-black uppercase tracking-wider" style={{ color: "var(--th-text-primary)" }}>
          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-500/15 text-sm" style={{ boxShadow: "0 0 8px rgba(59,130,246,0.3)" }}>
            🏛️
          </span>
          {t({ ko: "부서 성과", en: "DEPT. PERFORMANCE", ja: "部門パフォーマンス", zh: "部门绩效" })}
          <span className="ml-auto text-[9px] font-medium normal-case tracking-normal" style={{ color: "var(--th-text-muted)" }}>
            {t({ ko: "부서별", en: "by department", ja: "部門別", zh: "按部门" })}
          </span>
        </h2>
        ...
        {deptData.length === 0 ? (
          <div className="flex min-h-[200px] flex-col items-center justify-center gap-2 text-sm" style={{ color: "var(--th-text-muted)" }}>
            ...
            {t({ ko: "데이터 없음", en: "No data available", ja: "データなし", zh: "暂无数据" })}
          </div>
        ) : (
            ...
            {deptData.map((dept) => (
              ...
              >
                ...
                <div className="mt-1.5 flex justify-between text-[9px] font-semibold uppercase tracking-wider" style={{ color: "var(--th-text-muted)" }}>
                  <span>
                    {t({ ko: "완료", en: "cleared", ja: "完了", zh: "已完成" })} {numberFormatter.format(dept.done)}
                  </span>
                  <span>
                    {t({ ko: "전체", en: "total", ja: "合計", zh: "总计" })} {numberFormatter.format(dept.total)}
                  </span>
                </div>
              </article>
            ))}
      ...
      <div className="game-panel p-5">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-wider" style={{ color: "var(--th-text-primary)" }}>
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-cyan-500/15 text-sm" style={{ boxShadow: "0 0 8px rgba(0,240,255,0.2)" }}>
              🤝
            </span>
            ...
            return (
              <div
                ...
                title={`${localeName(language, agent)} · ${
                  isWorking ? t({ ko: "작업 중", en: "Working", ja: "作業中", zh: "工作中" }) : t({ ko: "대기 중", en: "Idle", ja: "待機", zh: "待机" })
                } · ${tier.name}`}
                ...
                <div className="relative">
                  ...
                  <span
                    className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 ${
                      isWorking ? "animate-status-glow bg-emerald-400" : "bg-slate-600"
                    }`}
                    style={{ borderColor: "var(--th-bg-primary)" }}
                  />
                </div>
                <span className="max-w-[52px] truncate text-center text-[9px] font-bold leading-tight" style={{ color: isWorking ? "var(--th-text-primary)" : "var(--th-text-muted)" }}>
...
export function DashboardMissionLog({
  ...
  return (
    <div className="game-panel p-5">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-wider" style={{ color: "var(--th-text-primary)" }}>
          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-violet-500/15 text-sm" style={{ boxShadow: "0 0 8px rgba(139,92,246,0.2)" }}>
            📜
          </span>
        ...
        <div className="space-y-2">
          {recentTasks.map((task) => {
            ...
            return (
              <article
                key={task.id}
                className={`group grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-xl border border-white/[0.06] border-l-[3px] ${leftBorder} bg-white/[0.02] p-3 transition-all duration-200 hover:translate-x-1 hover:bg-white/[0.04]`}
              >
                {assignedAgent ? (
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 flex items-center gap-1.5 text-[10px]" style={{ color: "var(--th-text-muted)" }}>
                    ...
                    {assignedAgent
                      ...
                      : t({ ko: "미배정", en: "Unassigned", ja: "未割当", zh: "未分配" })}
                  </p>
                </div>
