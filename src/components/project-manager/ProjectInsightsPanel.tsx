...
function getTaskStatusLabel(status: string, t: ProjectI18nTranslate): string {
  const labels: Record<string, string> = {
    inbox: t({ ko: "인입", en: "Inbox", ja: "Inbox", zh: "Inbox" }),
    pending: t({ ko: "대기", en: "Pending", ja: "Pending", zh: "Pending" }),
    planned: t({ ko: "계획됨", en: "Planned", ja: "Planned", zh: "Planned" }),
    ...
    cancelled: t({ ko: "취소", en: "Cancelled", ja: "Cancelled", zh: "Cancelled" }),
...
export default function ProjectInsightsPanel({
  ...
  async function handleScanPromotions() {
    ...
    try {
      ...
      setPromotionActionMessage("전사 공통 Skill 후보 재스캔에 실패했습니다.");
    } finally {
    ...
    <div className="min-w-0 space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              {t({
                ko: "프로젝트 운영 상태, 작업 이력, 기억, 승인 흐름을 한 곳에서 확인합니다.",
                en: "Review project operations, task history, memory, and approval flow in one place.",
                ja: "Review project operations, task history, memory, and approval flow in one place.",
                zh: "Review project operations, task history, memory, and approval flow in one place.",
              })}
            </p>
          </div>
      ...
      {activeView === "overview" && (
        <div className="space-y-4">
          <Panel title={t({ ko: "프로젝트 정보", en: "Project Info", ja: "Project Info", zh: "Project Info" })}>
            {loadingDetail ? (
              <p className="text-xs text-slate-400">{t({ ko: "불러오는 중입니다.", en: "Loading...", ja: "Loading...", zh: "Loading..." })}</p>
            ) : isCreating ? (
              <p className="text-xs text-slate-500">
                {t({ ko: "새 프로젝트를 입력 중입니다.", en: "Creating a new project", ja: "Creating a new project", zh: "Creating a new project" })}
              </p>
            ) : !selectedProject ? (
              <p className="text-xs text-slate-500">{t({ ko: "프로젝트를 선택하세요.", en: "Select a project", ja: "Select a project", zh: "Select a project" })}</p>
            ) : (
          ...
          <Panel title={t({ ko: "최근 작업", en: "Recent Tasks", ja: "Recent Tasks", zh: "Recent Tasks" })}>
            {!selectedProject ? (
              <p className="text-xs text-slate-500">-</p>
            ) : groupedTaskCards.length === 0 ? (
              <p className="text-xs text-slate-500">
                {t({ ko: "연결된 작업이 없습니다.", en: "No mapped tasks", ja: "No mapped tasks", zh: "No mapped tasks" })}
              </p>
            ) : (
              <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
                {groupedTaskCards.slice(0, 8).map((group) => (
                  <TaskButton key={group.root.id} task={group.root} t={t} onOpenTaskDetail={handleOpenTaskDetail} />
                ))}
              </div>
            )}
          </Panel>

          <Panel title={t({ ko: "적용된 모듈", en: "Applied Modules", ja: "Applied Modules", zh: "Applied Modules" })}>
            {!selectedProject ? (
              <p className="text-xs text-slate-500">-</p>
            ) : projectModulesLoading ? (
              <p className="text-xs text-slate-400">
                {t({ ko: "모듈 상태를 불러오는 중입니다.", en: "Loading module state", ja: "Loading module state", zh: "Loading module state" })}
              </p>
            ) : projectModules.length === 0 ? (
              <p className="text-xs text-slate-500">
                {t({ ko: "아직 프로젝트에 적용된 모듈이 없습니다.", en: "No modules applied yet", ja: "No modules applied yet", zh: "No modules applied yet" })}
              </p>
            ) : (
      ...
      {activeView === "board" && (
        <div className="grid gap-4 lg:grid-cols-3">
          {boardViewColumns.map((column) => (
            <Panel key={column.key} title={column.title} count={boardColumns[column.key].length}>
      ...
      {activeView === "gantt" && (
        <Panel title={t({ ko: "간트 타임라인", en: "Gantt Timeline", ja: "Gantt Timeline", zh: "Gantt Timeline" })}>
          <div className="space-y-3">
            {ganttItems.length === 0 ? (
              <p className="text-xs text-slate-500">
                {t({ ko: "표시할 일정이 없습니다.", en: "No schedule data", ja: "No schedule data", zh: "No schedule data" })}
              </p>
            ) : (
              ganttItems.map((item) => (
      ...
      {activeView === "reports" && (
        <div className="grid gap-4 xl:grid-cols-2">
          <Panel title={t({ ko: "보고 이력", en: "Report History", ja: "Report History", zh: "Report History" })}>
            <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
              {sortedReports.length === 0 ? (
                <p className="text-xs text-slate-500">
                  {t({ ko: "연결된 보고가 없습니다.", en: "No mapped reports", ja: "No mapped reports", zh: "No mapped reports" })}
                </p>
              ) : (
          ...
          <Panel title={t({ ko: "의사결정 이력", en: "Decision History", ja: "Decision History", zh: "Decision History" })}>
            <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
              {sortedDecisionEvents.length === 0 ? (
                <p className="text-xs text-slate-500">
                  {t({ ko: "기록된 의사결정이 없습니다.", en: "No decision records", ja: "No decision records", zh: "No decision records" })}
                </p>
              ) : (
      ...
      {activeView === "memory" && (
        <div className="space-y-4">
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,0.9fr)]">
            <Panel title="프로젝트 기억" badge={projectMemoryLoading ? "동기화 중" : undefined}>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="space-y-4">
              <Panel title="Beads 연동 상태">
                <div className="space-y-2 text-xs text-slate-300">
                  <InfoRow label="상태" value={beadsStatusLabel(projectMemory)} />
                  <InfoRow label="CLI 설치" value={projectMemory?.beads_status?.installed ? "감지됨" : "미감지"} />
                  <InfoRow label="프로젝트 초기화" value={projectMemory?.beads_status?.initialized ? "연결됨" : "미연결"} />
                  <InfoRow label="ready 목록" value={String(projectMemory?.beads_status?.ready_count ?? "-")} />
                  <InfoRow
                    label="Outbox 대기"
                    value={String((projectMemory?.memory_outbox ?? []).filter((item) => item.status === "pending").length)}
                  />
                  <InfoRow
                    label="Outbox 실패"
                    value={String((projectMemory?.memory_outbox ?? []).filter((item) => item.status === "failed").length)}
                  />
                  <button
                    type="button"
              ...
              <Panel
                ...
                action={
                  ...
                  >
                    {promotionBusyId === "scan" ? "재스캔 중" : "후보 재스캔"}
                  </button>
                }
              >
                <div className="space-y-2">
                  {visiblePromotionCandidates.length === 0 ? (
                    <p className="text-xs text-slate-500">
                      승인 대기 후보가 없습니다. 여러 프로젝트에서 반복 성공한 skill이 쌓이면 표시합니다.
                    </p>
                  ) : (
      ...
      {activeView === "rollout20" && (
        <div className="space-y-4">
          <Panel
            title={t({ ko: "Rollout 20 진행률", en: "Rollout 20 Progress", ja: "Rollout 20 Progress", zh: "Rollout 20 Progress" })}
            badge={rollout20.sampleMode ? t({ ko: "샘플 데이터", en: "Sample Data", ja: "Sample Data", zh: "Sample Data" }) : undefined}
          >
          ...
          <Panel title={t({ ko: "단계 타임라인", en: "Step Timeline", ja: "Step Timeline", zh: "Step Timeline" })}>
            <div className="space-y-2">
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/40 p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {badge ? <span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-[11px] text-amber-200">{badge}</span> : null}
          {action}
        </div>
      </div>
      {children}
...
function TaskButton({
  ...
  return (
    ...
    >
      <p className="break-all text-xs font-semibold text-slate-100">{task.title}</p>
      <p className="mt-1 text-[11px] text-slate-400">
        {getTaskTypeLabel(task.task_type, t)} - {getTaskStatusLabel(task.status, t)} - {fmtTime(task.updated_at || task.created_at)}
      </p>
    </button>
  );
}
