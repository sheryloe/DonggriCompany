    ...
    >
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: "var(--th-text-primary)" }}>
          {t({ ko: "CLI 계정/실행 상태", en: "CLI Account & Runner", ja: "CLI アカウント/実行状態", zh: "CLI 账号/运行状态" })}
        </h3>
      ...
      <div className="grid gap-2 rounded-lg border border-slate-700/60 bg-slate-900/30 p-3 text-xs sm:grid-cols-3">
        <div>
          <div className="text-slate-400">{t({ ko: "활성 Runner", en: "Active runners", ja: "稼働 Runner", zh: "活跃 Runner" })}</div>
        ...
        <div>
          <div className="text-slate-400">{t({ ko: "Docker 실행", en: "Docker mode", ja: "Docker 実行", zh: "Docker 模式" })}</div>
          ...
          return (
            <article key={provider} className="space-y-3 rounded-lg border border-slate-700/60 bg-slate-800/40 p-3">
              <div className="flex items-center gap-3">
                ...
                <span className={`rounded-full px-2 py-0.5 text-[11px] ${cliTool?.installed ? "bg-emerald-500/15 text-emerald-300" : "bg-slate-600/40 text-slate-300"}`}>
                  {cliTool?.installed
                ...
                <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
                  ...
                  <button
                    ...
                    disabled={
                      isBusy ||
                      !labelDraft.trim() ||
                      labelDraft.trim() === selectedPool.label
                    }
                    onClick={() =>
                      void (async () => {
              ...
              <div className="grid gap-2 text-xs sm:grid-cols-2">
                <div className="rounded border border-slate-700/50 bg-slate-900/30 p-2">
                  <div className="text-slate-400">{t({ ko: "계정 상태", en: "Account status", ja: "アカウント状態", zh: "账号状态" })}</div>
                  {selectedPool ? (
                    <div className="mt-1 flex items-center gap-2">
                      <span className={`rounded-full px-2 py-0.5 text-[11px] ${statusChipClass(selectedPool.status)}`}>
                        {selectedPool.status === "connected" && t({ ko: "연결됨", en: "Connected", ja: "接続済み", zh: "已连接" })}
                        {selectedPool.status === "auth_required" &&
                          t({ ko: "인증 필요", en: "Auth required", ja: "認証必要", zh: "需要认证" })}
                        {selectedPool.status === "install_required" &&
                          t({ ko: "CLI 설치 필요", en: "Install required", ja: "CLI インストール必要", zh: "需要安装 CLI" })}
                    ...
                    <div className="mt-1 text-slate-500">{t({ ko: "선택된 계정풀이 없습니다.", en: "No selected pool.", ja: "選択されたプールがありません。", zh: "未选择账号池。" })}</div>
                  )}
                </div>

                <div className="rounded border border-slate-700/50 bg-slate-900/30 p-2">
                  <div className="text-slate-400">{t({ ko: "Runner 상태", en: "Runner status", ja: "Runner 状態", zh: "Runner 状态" })}</div>
                  {runner ? (
                    <div className="mt-1 flex items-center gap-2">
                      <span className={`rounded-full px-2 py-0.5 text-[11px] ${runnerChipClass(runner.status)}`}>{runner.status}</span>
                      <span className="text-slate-500">Q:{queuedCount}</span>
                    </div>
                  ) : (
                    <div className="mt-1 text-slate-500">{t({ ko: "Runner 없음", en: "No runner", ja: "Runner なし", zh: "无 Runner" })}</div>
                  )}
                </div>
              </div>
              ...
              <div className="grid gap-2 sm:grid-cols-4">
                <button
                  ...
                  onClick={() =>
                    selectedPool &&
                    void onCopyLoginCommand(provider, selectedPool.accountPoolId)
                  }
              ...
              {selectedPool && verifyMessageByKey[`${provider}:${selectedPool.accountPoolId}`] && (
                <p className="text-[11px] text-slate-400">
                  {t({ ko: "최근 검증 결과", en: "Last verify result", ja: "最新検証結果", zh: "最近验证结果" })}: {verifyMessageByKey[`${provider}:${selectedPool.accountPoolId}`]}
                </p>
              )}

              <div className="grid gap-2 sm:grid-cols-[120px_1fr] sm:items-center">
                <span className="text-xs text-slate-400">{t({ ko: "모델", en: "Model", ja: "モデル", zh: "模型" })}</span>
                {cliModelsLoading ? (
                  <span className="text-xs text-slate-500">{t({ ko: "모델 로딩 중...", en: "Loading models...", ja: "モデル読込中...", zh: "模型加载中..." })}</span>
                  ...
                  <span className="text-xs text-slate-500">{t({ ko: "모델 목록 없음", en: "No models", ja: "モデルなし", zh: "无模型" })}</span>
                )}
              </div>
            </article>
          );
