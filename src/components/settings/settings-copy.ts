...
export function getGatewaySettingsCopy(t: TFunction) {
  return {
    ...
    intro: t({
      ko: "이 탭에서 메신저 채널을 직접 설정할 수 있습니다. 채팅 추가로 메신저, 토큰, 대상 ID, 연결 에이전트를 등록하세요.",
...
export function getOauthSettingsCopy(t: TFunction) {
  return {
    ...
    cliDetected: t({ ko: "CLI 감지", en: "CLI detected" }),
    ...
    copilotSubscriptionHint: t({
      ko: "GitHub Copilot 구독이 있어야 모델 실행이 가능합니다. 저장소 가져오기만 필요하면 무시해도 됩니다.",
