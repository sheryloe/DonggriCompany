    ...
    <div className="skills-learn-modal fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/75 backdrop-blur-sm p-4">
      <div className="skills-learn-modal-card w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/95 shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-700/60 px-5 py-4">
          <div>
            <h3 className="text-base font-semibold text-white">{skillText(t, "learning.title")}</h3>
          ...
          >
            {learnInProgress ? skillText(t, "learning.running") : skillText(t, "action.close")}
          </button>
        </div>
        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4 max-h-[calc(90vh-72px)]">
          <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/8 px-3 py-2">
            <div className="text-[11px] text-emerald-200">{skillText(t, "learning.installCommand")}</div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs text-slate-400">{skillText(t, "learning.selectProviders")}</div>
            <div className="text-[11px] text-slate-500">
              {selectedProviders.length}
              {skillText(t, "learning.selectedCount")}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {representatives.map((row) => {
              ...
              const displayName = row.agent
                ...
                : skillText(t, "learning.noAssignedMember");
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    <div
                      className={`relative ${isAnimating ? "learn-avatar-reading" : ""} ${
                        isHitAnimating ? "unlearn-avatar-hit" : ""
                      }`}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-slate-500">
                        {row.agent ? roleLabel(row.agent.role, t) : skillText(t, "learning.unavailable")}
                      </div>
                    </div>
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-1.5">
                      ...
                      >
                        {isAlreadyLearned
                          ? skillText(t, "learning.learned")
                          : isSelected
                            ? skillText(t, "learning.selected")
                            : skillText(t, "learning.idle")}
                      </div>
                      {isAlreadyLearned && (
                        <button
                          type="button"
                        ...
                        >
                          {isUnlearning ? skillText(t, "learning.unlearning") : skillText(t, "learning.unlearn")}
                        </button>
                      )}
                    </div>
                  </div>
          ...
          <div className="rounded-xl border border-slate-700/70 bg-slate-800/55 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <div className="text-slate-300">
                {skillText(t, "learning.jobStatus")}:{" "}
                <span
            ...
            {learnJob && (
              <div className="mt-2 rounded-lg border border-slate-700 bg-slate-900/70 p-2 font-mono text-[10px] text-slate-300 max-h-32 overflow-y-auto space-y-1">
                ...
                {learnJob.logTail.length > 0 ? (
                  ...
                  <div className="text-slate-600">{skillText(t, "learning.noLogs")}</div>
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              {learnSubmitting || learnInProgress
                ? skillText(t, "learning.learning")
                : skillText(t, "learning.startLearning")}
            </button>
          </div>
        </div>
      </div>
