...
function spriteSvg(spriteNumber, direction, frame) {
  const palette = palettes[(spriteNumber - 1) % palettes.length];
  const variation = Math.floor((spriteNumber - 1) / palettes.length);
  const offset = frameOffsets[direction][frame - 1];
  const cx = 48 + offset.x;
  const lift = offset.y;
  const stripe = variation % 2 === 0 ? palette.accent : palette.shirt;
  const badge = variation % 3 === 0 ? palette.accent : "#f6d365";
  const side = direction === "L" ? -1 : direction === "R" ? 1 : 0;
  const isBack = direction === "B";
  const isSide = direction === "L" || direction === "R";
  const eyeX = isSide ? cx + side * 7 : cx;

  const arms =
    direction === "L"
      ? `${rect(cx - 25, 45 + lift, 9, 25, palette.jacket)}${rect(cx + 16, 47 + lift, 7, 20, palette.jacket)}`
      : direction === "R"
        ? `${rect(cx - 23, 47 + lift, 7, 20, palette.jacket)}${rect(cx + 16, 45 + lift, 9, 25, palette.jacket)}`
        : `${rect(cx - 25, 45 + lift, 8, 22, palette.jacket)}${rect(cx + 17, 45 + lift, 8, 22, palette.jacket)}`;

  const face = isBack
    ? `${rect(cx - 18, 20 + lift, 36, 17, palette.hair)}${rect(cx - 15, 37 + lift, 30, 6, "#24303d")}`
    : isSide
      ? `${rect(cx - 15, 22 + lift, 27, 22, palette.skin)}${rect(cx - 16, 19 + lift, 22, 8, palette.hair)}${rect(
          eyeX,
          33 + lift,
          3,
          3,
          "#26313c",
        )}${rect(cx + side * 10, 38 + lift, 4, 2, "#9d5f55")}`
      : `${rect(cx - 16, 22 + lift, 32, 22, palette.skin)}${rect(cx - 18, 19 + lift, 36, 8, palette.hair)}${rect(
          cx - 8,
          33 + lift,
          3,
          3,
          "#26313c",
        )}${rect(cx + 5, 33 + lift, 3, 3, "#26313c")}${rect(cx - 4, 39 + lift, 8, 2, "#9d5f55")}`;

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${spriteSize}" height="${spriteSize}" viewBox="0 0 ${spriteSize} ${spriteSize}" shape-rendering="crispEdges">
    ${ellipse(48, 85, 18, 3, "#101827", 'opacity="0.20"')}
    ${rect(cx - 11, 64 + lift, 8, 17, palette.pants)}
    ${rect(cx + 3, 64 - lift, 8, 17, palette.pants)}
    ${rect(cx - 14, 82 + lift, 13, 5, "#1b2431")}
    ${rect(cx + 1, 82 - lift, 13, 5, "#1b2431")}
    ${arms}
    ${rect(cx - 19, 42 + lift, 38, 26, palette.jacket)}
    ${rect(cx - 11, 42 + lift, 22, 26, palette.shirt)}
    ${rect(cx - 2, 42 + lift, 4, 26, stripe)}
    ${rect(cx + 8, 48 + lift, 5, 5, badge)}
    ${rect(cx - 18, 38 + lift, 36, 6, palette.jacket)}
    ${face}
    ${rect(cx - 14, 15 + lift, 28, 7, palette.hair)}
    ${rect(cx - 10, 13 + lift, 20, 4, palette.hair)}
  </svg>`;
}
  ...
  for (let spriteNumber = 1; spriteNumber <= runtimeSpriteCount; spriteNumber += 1) {
    for (const direction of directions) {
      for (const frame of frames) {
        const outputPath = path.join(spritesDir, `${spriteNumber}-${direction}-${frame}.png`);
        await sharp(Buffer.from(spriteSvg(spriteNumber, direction, frame)))
          .png()
          .toFile(outputPath);
        count += 1;
      }
...
async function updateManifests(spriteCount) {
  const manifest = {
    ...
    status: "generated_v2_runtime_assets",
    generated_at_kst: "2026-07-13",
    source: {
      type: "deterministic_runtime_generation_from_visual_v2_styleboard",
      styleboard: "public/generated/donggri-visual-v2/styleboard/donggri-visual-v2-styleboard.png",
    ...
    sprite_contract: {
      ...
      url_example: "/sprites/donggri-visual-v2/12-B-3.png?v=donggri-visual-v2",
    },
    replacement_policy: {
  ...
  publicManifest = {
    ...
    compatibility: {
      ...
      v2_runtime_replaces_seeded_copy: true,
